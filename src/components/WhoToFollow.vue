<template>
    <list-card label="Who to follow" show-more="/follows">
        <div class="wtf-item flex-row-space-between-center"
            ...
            <button
                    v-if="!followed.includes(list.user_name)"
                    @click="followRequest(list.user_name)"
                    class="follow-btn flex-row-center-center">
                Follow
            </button>
            <input
                    v-else
                    type="button"
                    @mouseenter="unfollowBtnHoverTrigger=list.user_name"
                    @mouseout="unfollowBtnHoverTrigger=null"
                    @click="followRequest(list.user_name)"
                    :class="{'unfollow-btn-onhover': unfollowBtnHoverTrigger === list.user_name}"
                    class="follow-btn unfollow-btn flex-row-center-center"
